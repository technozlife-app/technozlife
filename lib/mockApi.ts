...
import mockData, { generateRecommendations as clientGenerateRecommendations } from "./mock-data";
import { Habit, HabitEntry, WearableDevice, DeviceMetric, Recommendation, GenerationJob } from "./models";
...
export const devicesApi = {
  ...
  sync(id: string, options: { days?: number } = { days: 7 }): DeviceMetric[] {
    ...
    for (let i = 0; i < days; i++) {
      ...
      metrics.push({ id: makeId(), deviceId: dev.id, type: "steps", value: Math.round(2000 + Math.random() * 8000), timestamp: t.toISOString() });
      metrics.push({ id: makeId(), deviceId: dev.id, type: "sleep", value: Math.round(5 + Math.random() * 4 * 10) / 10, timestamp: t.toISOString() });
    }

    const existing = read<DeviceMetric>(LS.deviceMetrics);
    write(LS.deviceMetrics, existing.concat(metrics));

    const updated = devices.map((d) => (d.id === id ? { ...d, lastSync: new Date().toISOString() } : d));
    write(LS.devices, updated);

    return metrics;
  },
  metricsForDevice(deviceId: string): DeviceMetric[] {
    return read<DeviceMetric>(LS.deviceMetrics).filter((m) => m.deviceId === deviceId);
  },
};
...
export const habitsApi = {
  ...
  add(data: Partial<Habit>): Habit {
    ...
    const h: Habit = { id: makeId(), name: data.name || "New Habit", schedule: data.schedule || "daily", createdAt: now, updatedAt: now, streak: data.streak || 0 };
    items.push(h);
    write(LS.habits, items);
    return h;
  },
  update(id: string, patch: Partial<Habit>) {
    const items = read<Habit>(LS.habits).map((h) => (h.id === id ? { ...h, ...patch, updatedAt: new Date().toISOString() } : h));
    write(LS.habits, items);
  ...
  remove(id: string) {
    ...
    write(LS.habits, items);
    const entries = read<HabitEntry>(LS.habitEntries).filter((e) => e.habitId !== id);
    write(LS.habitEntries, entries);
    return true;
  },
  entries(habitId: string): HabitEntry[] {
    return read<HabitEntry>(LS.habitEntries).filter((e) => e.habitId === habitId);
  },
  toggleEntry(habitId: string, date: string) {
    const entries = read<HabitEntry>(LS.habitEntries);
    const day = new Date(date).toISOString();
    const existing = entries.find((e) => e.habitId === habitId && e.date === day);
    if (existing) {
      const updated = entries.map((e) => (e.id === existing.id ? { ...e, completed: !e.completed } : e));
      write(LS.habitEntries, updated);
      return updated.find((e) => e.id === existing.id);
    } else {
      const e: HabitEntry = { id: makeId(), habitId, date: day, completed: true };
      entries.push(e);
      write(LS.habitEntries, entries);
      return e;
    }
...
export const recommendationsApi = {
  async generateFromData(dailyData: any[]): Promise<Recommendation[]> {
    try {
      ...
      if (res.status === "success" && res.data) {
        ...
        try {
          ...
          return parsed.map((p) => ({ ...p, source: "ai", createdAt: new Date().toISOString() }));
        } catch (e) {
          return [{ id: makeId(), title: "AI Recommendation", description: String(content || ""), confidence: 0.6, priority: "medium", source: "ai", createdAt: new Date().toISOString() }];
        }
      }
    } catch (e) {
    ...
    const recs = clientGenerateRecommendations(dailyData as any[]).map((r) => ({ ...r, source: "rule-based", createdAt: new Date().toISOString() })) as Recommendation[];
    return recs;
  },
};
...
export const jobsApi = {
  ...
  add(job: Partial<GenerationJob>): GenerationJob {
    ...
    const j: GenerationJob = { id: makeId(), prompt: job.prompt || "", status: job.status || "processing", createdAt: new Date().toISOString(), result: job.result, tokensUsed: job.tokensUsed };
    items.push(j);
    write(LS.jobs, items);
    return j;
  },
  update(id: string, patch: Partial<GenerationJob>) {
    const items = read<GenerationJob>(LS.jobs).map((j) => (j.id === id ? { ...j, ...patch } : j));
    write(LS.jobs, items);
