...
export function AIGenerator() {
  ...
  const handleGenerate = async () => {
    if (!prompt.trim()) {
      addToast("warning", "Empty Prompt", "Please enter a prompt to generate content")
      return
    }
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        addToast("success", "Content Generated", `Used ${response.data.tokensUsed} tokens`)
      } else {
      ...
      addToast("error", "Generation Failed", "Unable to generate content. Please try again.")
    } finally {
    ...
    >
      ...
      <div className="space-y-4">
        ...
        <Button
          ...
          className="w-full bg-gradient-to-r from-teal-500 to-emerald-500 text-slate-950 hover:from-teal-400 hover:to-emerald-400"
        >
          {isGenerating ? (
            <>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="relative">
            <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-slate-500">Generated Content</span>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={handleCopy}
