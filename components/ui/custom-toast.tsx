...
const toastConfig: Record<ToastType, { icon: typeof CheckCircle2; gradient: string; glow: string }> = {
  success: {
    icon: CheckCircle2,
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start gap-3">
                  <div
                    className={`flex-shrink-0 w-8 h-8 rounded-full bg-gradient-to-br ${config.gradient} flex items-center justify-center`}
                  >
                    <Icon className="w-4 h-4 text-white" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-white text-sm">{toast.title}</p>
                    {toast.message && <p className="text-sm text-slate-400 mt-0.5 line-clamp-2">{toast.message}</p>}
                  </div>
                </div>
                ...
                <motion.div
                  ...
                  className={`absolute bottom-0 left-1 right-0 h-0.5 origin-left bg-gradient-to-r ${config.gradient} rounded-b-2xl opacity-50`}
                />
              </motion.div>
            )
          })}
