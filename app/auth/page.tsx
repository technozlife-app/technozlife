...
import { ArrowLeft, Mail, Lock, User, Eye, EyeOff, Fingerprint, Sparkles, ChevronRight, Github } from "lucide-react"
...
export default function AuthPage() {
    ...
    try {
      if (mode === "signup") {
        if (!formData.agreeTerms) {
          addToast("error", "Terms Required", "Please agree to the Terms of Service and Privacy Policy")
          setIsLoading(false)
          return
        }
        ...
        if (result.success) {
          addToast("success", "Account Created", "Welcome to Technozlife! Redirecting to dashboard...")
          setTimeout(() => router.push("/dashboard"), 1500)
        } else {
          addToast("error", "Registration Failed", result.message || "Please check your details and try again")
        }
      } else {
        const result = await login(formData.email, formData.password)
        if (result.success) {
          addToast("success", "Welcome Back", "Successfully signed in. Redirecting...")
          setTimeout(() => router.push("/dashboard"), 1500)
        } else {
          addToast("error", "Sign In Failed", result.message || "Invalid email or password")
        }
      }
    } catch {
      addToast("error", "Connection Error", "Unable to connect to server. Please try again.")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleGoogleAuth = async () => {
    if (!GOOGLE_CLIENT_ID) {
      addToast("warning", "Google Auth", "Google authentication is being configured")
      ...
      try {
        ...
        if (result.success) {
          ...
          addToast("error", "Google Auth Failed", result.message || "Unable to authenticate with Google")
        }
      } catch {
  ...
  const handleGithubAuth = async () => {
    if (!GITHUB_CLIENT_ID) {
      addToast("warning", "GitHub Auth", "GitHub authentication is being configured")
      // Simulate OAuth flow for demo
      ...
      try {
        ...
        if (result.success) {
          addToast("success", "GitHub Sign In", "Successfully authenticated with GitHub")
          setTimeout(() => router.push("/dashboard"), 1500)
        } else {
          addToast("error", "GitHub Auth Failed", result.message || "Unable to authenticate with GitHub")
        }
      } catch {
        addToast("error", "Connection Error", "Unable to connect to server")
      } finally {
        ...
        <div className="hidden lg:flex lg:w-1/2 flex-col justify-center px-16 xl:px-24">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <Link href="/" className="inline-block mb-12">
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <div className="absolute inset-0 rounded-xl bg-gradient-to-br from-teal-400 to-emerald-500 blur-lg opacity-50" />
                </div>
                <span className="text-2xl font-bold text-white">Technozlife</span>
              </div>
            </Link>
            ...
            <p className="text-xl text-slate-400 leading-relaxed mb-12 max-w-lg">
              Join thousands who have already enhanced their potential through our bio-digital fusion platform.
            </p>
          ...
          >
            ...
            <Link href="/" className="lg:hidden inline-block mb-8">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xl font-bold text-white">Technozlife</span>
              </div>
            </Link>

            {/* Card */}
            <div className="relative">
              <div className="absolute -inset-1 bg-gradient-to-r from-teal-500/20 to-violet-500/20 rounded-3xl blur-xl opacity-50" />
              ...
              <div className="relative bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-2xl p-8">
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    <Button
                      type="submit"
                      disabled={isLoading}
                      className="w-full h-12 bg-gradient-to-r from-teal-500 to-emerald-500 hover:from-teal-400 hover:to-emerald-400 text-slate-950 font-semibold rounded-xl transition-all relative overflow-hidden group"
                    >
                      {isLoading ? (
                        <motion.div
                          animate={{ rotate: 360 }}
                ...
                <div className="relative my-8">
                  ...
                  <div className="relative flex justify-center">
                    <span className="px-4 bg-slate-900 text-sm text-slate-500">or continue with</span>
                  </div>
                </div>
            ...
            <p className="text-center text-sm text-slate-500 mt-6">
              {mode === "login" ? (
                <>
                  New to Technozlife?{" "}
                  <button onClick={() => setMode("signup")} className="text-teal-400 hover:underline">
                    Create an account
                  </button>
                </>
              ) : (
                <>
                  ...
                  <button onClick={() => setMode("login")} className="text-teal-400 hover:underline">
                    Sign in
                  </button>
                </>
              )}
